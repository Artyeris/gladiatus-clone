...
function ShopSlot({
  ...
  if (!slot.item) {
    return (
      <div className='flex flex-col items-center justify-center p-2 rounded-sm bg-brown2/10 text-xs opacity-50'
  ...
  return (
    <div
      className='flex flex-col items-center gap-1 p-2 rounded-sm bg-brown2/10'
      style={{ minHeight: '120px' }}
    >
      ...
      <div className='text-xs font-semibold text-center leading-tight' style={{ color: nameColor }}>
        {fullItemName(slot.item)}
      </div>
      <div className='text-xs opacity-80'>lvl {slot.item.level}</div>
      <button
        type='button'
        onClick={onBuy}
        disabled={disabled}
        className={`general-button px-2 py-1 rounded-sm text-xs font-semibold w-full ${
