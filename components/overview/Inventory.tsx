...
interface InventoryProps {
  ...
  onStartDrag?: (x: number, y: number, item: ItemInterface) => void;
}
...
const Inventory = ({ inventory, onDropItem, onStartDrag }: InventoryProps) => {
        ...
        {rows.flatMap((row, rowIndex) => (
          row.map((cell, cellIndex) => (
            <div
              ...
              onDrop={() => onDropItem?.(rowIndex, cellIndex)}
              style={{ 
              position: 'relative',
              width: '40px', 
              height: '40px', 
            ...
            >
              {isItem(cell) && (
                <Image
                  ...
                  alt={cell.name}
                  draggable
                  onDragStart={() => onStartDrag?.(rowIndex, cellIndex, cell)}
